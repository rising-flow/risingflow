...
let selectedGame = null; // Store the selected game

// Function to show the game selection modal
function showGameSelectionModal() {
    const modal = document.getElementById('game-selection-modal');
    const gameList = document.getElementById('game-list');

    // Populate the game list dynamically
    const games = [
    ...
    games.forEach(game => {
        ...
        li.addEventListener('click', () => {
            selectedGame = game;
            localStorage.setItem('selectedGame', game); // Save to local storage
            modal.style.display = 'none';
            loadAllSongs(); // Reload songs for the selected game
        });
...
// Modify loadAllSongs to filter by selected game
async function loadAllSongs() {
    console.log("Starting loadAllSongs...");
    loadingMessage.textContent = 'Loading songs...';
    categoryListContainer.innerHTML = ''; // Clear previous content
    ...
    try {
        const fetchPromises = jsonFiles.map(async file => {
            ...
            // Filter songs by the selected game
            if (!selectedGame || categoryName === selectedGame) {
                allSongsData[categoryName] = data;
            }
            return { categoryName, data };
        });
...
// Event listener for the "Change Game" button
document.getElementById('change-game-btn').addEventListener('click', changeGame);

// Show the game selection modal on page load if no game is selected
document.addEventListener('DOMContentLoaded', () => {
    selectedGame = localStorage.getItem('selectedGame');
    if (!selectedGame) {
        showGameSelectionModal();
    } else {
        loadAllSongs(); // Load songs for the previously selected game
    }
});
